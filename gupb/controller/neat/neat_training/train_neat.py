...
from gupb.controller.neat.neat_training.neat_evaluator import NeatEvaluatorV1, NeatEvaluator, NeatEvaluatorVT1, NeatDistanceEvaluator
...
def eval_genomes_with_by_tick(evaluator_name, genomes, config, ticks_per_update=1):
    for genome_id, genome in genomes:
        ...
        play = True
        while play and not game.finished:
            game.cycle()
            tick_count += 1
            if not is_net_alive(neat_controller, game):
                play = False

            if tick_count % ticks_per_update == 0 and play:
                score = evaluator.calculate_score(game, tick_count)
                genome.fitness += 0.05 * score



def is_any_controller_alive(controllers, game):
    for champion in game.champions:
        if champion.controller in controllers:
            if champion.alive:
                return True
...
def is_net_alive(controller, game):
    for champion in game.champions:
